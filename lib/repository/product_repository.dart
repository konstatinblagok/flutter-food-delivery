...
import 'package:fluttermultistoreflutter/db/product_collection_dao.dart';
...
import 'package:fluttermultistoreflutter/viewobject/favourite_product.dart';
import 'package:fluttermultistoreflutter/viewobject/product_collection.dart';
import 'package:fluttermultistoreflutter/viewobject/related_product.dart';
...
import 'package:fluttermultistoreflutter/viewobject/product_map.dart';
...
class ProductRepository extends PsRepository {
  ...
  Future<dynamic> getProductDetail(
    ...
    if (isConnectedToInternet) {
      final PsResource<Product> _resource =
          await _psApiService.getProductDetail(productId, loginUserId, shopId);
  ...
  Future<dynamic> getRelatedProductList(
    ...
    // Prepare Holder and Map Dao
    // final String paramKey = holder.getParamKey();
     final Finder finder = Finder(filter: Filter.equals(mainProductIdKey, productId));
    ...
    sinkCollectionProductListStream(
        ...
        await _productDao.getAllDataListWithFilterId(productId, mainProductIdKey, relatedProductDao, RelatedProduct(),
            status: status));
            
    // Server Call
    if (isConnectedToInternet) {
      final PsResource<List<Product>> _resource = await _psApiService
          .getRelatedProductList(productId, categoryId, shopId, limit, offset);
